...
export default function AdminCouponsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [c, s, cats] = await Promise.all([
      fetch('/api/coupons?limit=100').then(r => r.json()),
  ...
  const columns = [
    ...
    { key: 'title', label: 'Title', render: v => <span className="text-sm font-medium text-[#1a1916] max-w-[180px] truncate block">{v}</span> },
    { key: 'code', label: 'Code', render: v => <span className="font-mono text-xs font-semibold bg-[#fdf0e0] text-[#d4720a] px-2 py-0.5 rounded-md tracking-wider">{v}</span> },
    { key: 'discount', label: 'Discount', render: v => <span className="text-xs font-display font-semibold text-[#6b6860]">{v}</span> },
  ...
  if (creating || editing) {
    return (
      <div className="p-6 max-w-3xl">
        ...
        <h1 className="font-display font-semibold text-xl text-[#1a1916] mb-6">{editing ? 'Edit Coupon' : 'New Coupon'}</h1>
        <div className="bg-white border border-[#e5e2db] rounded-xl p-5">
          <OfferForm type="coupon" initial={editing} stores={stores} categories={categories} onSave={handleSave} onCancel={() => { setCreating(false); setEditing(null); }} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display font-semibold text-xl text-[#1a1916]">Coupons</h1>
          <p className="text-sm text-[#9e9b96]">{coupons.length} total coupons</p>
        </div>
      ...
      {loading ? (
        ...
        <DataTable columns={columns} rows={coupons} onDelete={handleDelete} onEdit={setEditing} />
      )}
    </div>
  );
}
